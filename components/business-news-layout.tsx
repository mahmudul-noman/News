...
export function BusinessNewsLayout({ articles, title, categorySlug }: BusinessNewsLayoutProps) {
    const mainArticle = articles[0]
    // Left column: items 1-4 (4 items)
    const leftArticles = articles.slice(1, 5)
    // Right column: items 5-6 (2 items) - because we have an ad at the top
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-stretch">
                ...
                <div className="lg:col-span-1 flex flex-col order-2 lg:order-1 h-full lg:border-r lg:border-gray-200 lg:pr-6">
                    {leftArticles.map((article, index) => (
                        ...
                        >
                            <Link href={getArticleLink(article)} className="contents">
                                <div className="flex-grow order-1">
                                    <h3 className="font-medium text-lg leading-snug group-hover:text-red-600 transition-colors">
                                        {article.title}
                                    </h3>
                                </div>
                ...
                <div className="lg:col-span-2 order-1 lg:order-2 h-full lg:px-6">
                    <article className="group h-full flex flex-col">
                        <Link href={getArticleLink(mainArticle)} className="flex flex-col h-full">
                            ...
                            <div className="flex flex-col flex-shrink-0">
                                <h3 className="font-bold text-2xl md:text-3xl leading-tight group-hover:text-red-600 transition-colors mb-3">
                                    {mainArticle.title}
                                </h3>
                ...
                {/* Right Column: 1 Ad + 2 Small News Items */}
                <div className="lg:col-span-1 flex flex-col order-3 h-full lg:border-l lg:border-gray-200 lg:pl-6 gap-6">
                    {/* Ad Component */}
                    ...
                    <div className="flex flex-col h-full">
                        {rightArticles.map((article, index) => (
                            <article
                                key={article.id}
                                className={`group flex gap-4 items-center justify-between ${index !== rightArticles.length - 1 ? "border-b border-gray-200" : ""
                                    } py-4 first:pt-0 last:pb-0 h-full`}
                            >
                                <Link href={getArticleLink(article)} className="contents">
                                    <div className="flex-grow order-1">
